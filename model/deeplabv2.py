...
class ResNetMulti(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        if self.multi_level:
            x1 = self.layer5(x)  # produce segmap 1 # inchannel = 1024, out_channel = num_classes
        else:
            x1 = None
        x2 = self.layer4(x)   # inchannel= 1024, out_channel = 2048  x2 class feature
        x3 = self.layer6(x2)  # produce segmap 2 inchannel_2048 out_channel = num_classes
