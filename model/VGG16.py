...
class VGG16Modified(nn.Module):
    ...
    def __init__(self, n_classes=21,device=1):
    ...
    def to_tridiagonal_multidim(self, w,device):
        N, W, C, D = w.size()
        tmp_w = w / torch.sum(torch.abs(w), dim=3).unsqueeze(-1)
        tmp_w = tmp_w.unsqueeze(2).expand([N, W, W, C, D])

        eye_a = Variable(torch.diag(torch.ones(W - 1).cuda(device), diagonal=-1))
        eye_b = Variable(torch.diag(torch.ones(W).cuda(device), diagonal=0))
        eye_c = Variable(torch.diag(torch.ones(W - 1).cuda(device), diagonal=1))
    ...
    def forward(self, x, coarse_segmentation):
        ...
        rnn_h1 = Variable(torch.zeros((N, H, W, four_directions // 3)).cuda(self.device))
        rnn_h2 = Variable(torch.zeros((N, H, W, four_directions // 3)).cuda(self.device))
        rnn_h3 = Variable(torch.zeros((N, H, W, four_directions // 3)).cuda(self.device))
        rnn_h4 = Variable(torch.zeros((N, H, W, four_directions // 3)).cuda(self.device))
        ...
        for i in range(W):
            ...
            tmp_w = self.to_tridiagonal_multidim(tmp_w,self.device)  # N, H, W, 32
        ...
        for i in range(H):
            ...
            tmp_w = self.to_tridiagonal_multidim(tmp_w,self.device)  # N, W, H, 32
        ...
        rnn_h5 = Variable(torch.zeros((N, H, W, four_directions // 3)).cuda(self.device))
        rnn_h6 = Variable(torch.zeros((N, H, W, four_directions // 3)).cuda(self.device))
        rnn_h7 = Variable(torch.zeros((N, H, W, four_directions // 3)).cuda(self.device))
        rnn_h8 = Variable(torch.zeros((N, H, W, four_directions // 3)).cuda(self.device))
...
def get_vgg16(num_classes=19,device=1):
    model = VGG16Modified(num_classes,device=device)
    return model

# if __name__ == '__main__':
    # import torch
    # device = 1
    # model = get_vgg16(num_classes=5,device=device).cuda(1)
    # model.eval()
    # input = torch.rand(1, 3, 256, 256).cuda(1)
    # mask  = torch.rand(1,5,256,256).cuda(1)
    # mask  = torch.softmax(mask,dim=1)
    # refine_mask = model(input,mask)
    # print(mask.size())
